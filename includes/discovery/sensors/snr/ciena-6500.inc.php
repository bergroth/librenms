...
#	d_echo($entry);
	...
	if (is_numeric($index) )
	{
	        $oid = '.1.3.6.1.4.1.562.68.10.1.1.2.1.1.6.'.$index.'.43';
       		#$descr = dbFetchCell('SELECT `ifDescr` FROM `ports` WHERE `ifIndex`= ? AND `device_id` = ?', array($index, $device['device_id'])) . ' Rx Power';
		$descr = snmp_get($device, "nnOpticalPmRecentIfIndexDescr.$index.43", '-Oqv', 'NORTEL-OPTICAL-PM-MIB');
	        ...
	        $current = snmp_get($device, "nnOpticalPmUntValue.$index.43", '-Oqv', 'NORTEL-OPTICAL-PM-MIB');
		if (is_numeric($current)) {
			...
			$entPhysicalIndex = $index;
	        	...
	        	discover_sensor($valid['sensor'], 'snr', $device, $oid, 'rx-'.$index, 'ciena-6500', $descr, $divisor, $multiplier, $limit_low, $warn_limit_low, $warn_limit, $limit, $current, 'snmp', $entPhysicalIndex, $entPhysicalIndex_measured);
		}
	}
}
